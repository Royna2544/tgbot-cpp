...
template <>
std::shared_ptr<InputMediaVenue> parse(const nlohmann::json &data) {
    auto result = std::make_shared<InputMediaVenue>();
...
template <>
nlohmann::json put(const std::shared_ptr<InputMediaVenue> &object) {
    JsonWrapper json;
    if (object) {
