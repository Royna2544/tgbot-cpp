...
template <>
std::shared_ptr<PassportElementErrorReverseSide> parse(const nlohmann::json &data) {
...
template <>
nlohmann::json put(const std::shared_ptr<PassportElementErrorReverseSide> &object) {
    JsonWrapper json;
    if (object) {
