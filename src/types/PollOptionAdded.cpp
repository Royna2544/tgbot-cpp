...
template <>
std::shared_ptr<PollOptionAdded> parse(const nlohmann::json &data) {
    auto result = std::make_shared<PollOptionAdded>();
...
template <>
nlohmann::json put(const std::shared_ptr<PollOptionAdded> &object) {
    JsonWrapper json;
    if (object) {
